...
public class MovementSerenitySteps {
    ...
    public ValidatableResponse createMovement(String cameraDirectory, List<Double> entryLine, List<Double> exitLine, String name, int snapshotIndex, int analysisId){

        MovementClass movement = new MovementClass();
        movement.setCameraDirectory(cameraDirectory);
        movement.setEntryLine(entryLine);
        movement.setEntryLine(exitLine);
